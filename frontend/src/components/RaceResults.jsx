...
export default function RaceResults({onAgain}){
  ...
  return(<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.8)',backdropFilter:'blur(8px)'}}>
    <motion.div initial={{scale:.8,opacity:0}} animate={{scale:1,opacity:1}} transition={{type:'spring',stiffness:200}} className="glass" style={{width:460,padding:40,textAlign:'center',border:'1px solid rgba(255,51,102,.3)'}}>
      ...
      <div style={{fontFamily:'Orbitron',fontSize:14,color:'var(--gold)'}}>P{position}</div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,margin:'28px 0'}}>
        {[['BEST LAP',fmt(lapTime)],['TOTAL TIME',fmt(totalTime)],['POSITION',`P${position}`],['SCORE',score]].map(([l,v])=>(<div key={l} className="glass" style={{padding:14,borderTop:'2px solid var(--red)'}}><div style={{fontFamily:'Orbitron',fontSize:9,color:'var(--dim)',marginBottom:4}}>{l}</div><div style={{fontFamily:'Orbitron',fontSize:22,color:'var(--cyan)',fontWeight:700}}>{v}</div></div>))}
      </div>
