  ...
  return(<div style={{width:'100vw',height:'100vh',position:'relative',overflow:'hidden',background:'#0a0a0f'}}>
    ...
    <div style={{position:'relative',zIndex:10,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'100%',gap:32}}>
      <motion.div initial={{opacity:0,y:-40}} animate={{opacity:1,y:0}} transition={{duration:.8}} style={{textAlign:'center'}}>
        <div className="orb" style={{fontSize:'clamp(52px,9vw,100px)',fontWeight:900,lineHeight:1}}>
          <span className="nred">TURBO</span><span className="ncyan">RACE</span>
        </div>
        <div style={{fontFamily:'Rajdhani',fontSize:15,letterSpacing:'.4em',color:'var(--dim)',textTransform:'uppercase',marginTop:8}}>WebGL · Shaders · Multiplayer</div>
      </motion.div>
      ...
      <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.9}} style={{display:'flex',gap:48,marginTop:8}}>
        {[['3D SHADERS','WebGL Graphics'],['WASD DRIVE','Physics Engine'],['LEADERBOARD','Global Ranks'],['PROFILES','Race Stats']].map(([t,s])=>(
          <div key={t} style={{textAlign:'center'}}><div className="orb" style={{fontSize:10,fontWeight:700,color:'var(--red)',letterSpacing:'.1em'}}>{t}</div><div style={{fontSize:12,color:'var(--dim)',marginTop:4}}>{s}</div></div>
        ))}
      </motion.div>
    </div>
