...
const VS=`varying vec3 vN,vP;varying vec2 vUv;
void main(){vN=normalize(normalMatrix*normal);vP=(modelMatrix*vec4(position,1.)).xyz;vUv=uv;
gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`;
const FS=`uniform vec3 uC;uniform float uM,uT;varying vec3 vN,vP;varying vec2 vUv;
void main(){vec3 l=normalize(vec3(2.,5.,3.));float d=max(dot(vN,l),0.);
vec3 vd=normalize(cameraPosition-vP);vec3 h=normalize(l+vd);
float sp=pow(max(dot(vN,h),0.),128.)*uM;
float fr=pow(1.-max(dot(vN,vd),0.),3.)*.4;
vec3 col=uC*(0.15+d*.75)+vec3(sp)+uC*fr*.4;
col+=step(.45,fract(vUv.x*4.+uT*.25))*step(fract(vUv.x*4.+uT*.25),.55)*.1;
gl_FragColor=vec4(col,1.);}`;
const RVS=`varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`;
const RFS=`uniform float uT;varying vec2 vUv;
void main(){vec3 a=vec3(.12,.12,.14);float lane=step(.48,vUv.x)*step(vUv.x,.52);
float dash=step(.4,fract(vUv.y*10.-uT*.6))*lane;
float edge=(step(vUv.x,.015)+step(.985,vUv.x))*.6;
gl_FragColor=vec4(a+vec3(dash*.7)+vec3(edge*.5,edge*.35,0.),1.);}`;
...
function Car({pos,color,isPlayer,ctrl,id}){
  ...
  useFrame(st=>{
    ...
    if(isPlayer&&ctrl.current){
      const{w,a,s,d}=ctrl.current,mx=.3,ac=.011,fr=.965,ts=.04;
      ...
      if(body.current){const lean=(a?1:d?-1:0)*.05;body.current.rotation.z=THREE.MathUtils.lerp(body.current.rotation.z,lean,.1);}
    }else{
      const as=.15+id*.02,wb=Math.sin(t*.6+id*2.1)*.003;
  ...
  return(<group ref={g} position={pos}>
    <group ref={body}>
      <mesh castShadow><boxGeometry args={[1.8,.35,3.8]}/><shaderMaterial ref={sm} vertexShader={VS} fragmentShader={FS} uniforms={uni}/></mesh>
      <mesh position={[0,.45,.2]}><boxGeometry args={[1.4,.4,1.8]}/><shaderMaterial vertexShader={VS} fragmentShader={FS} uniforms={{uC:{value:cc},uM:{value:.7},uT:{value:0}}}/></mesh>
      <mesh position={[0,.55,.9]}><boxGeometry args={[1.35,.35,.05]}/><meshPhysicalMaterial color="#88ddff" transparent opacity={.6} roughness={0}/></mesh>
    ...
    {[[-1,-.2,1.3],[1,-.2,1.3],[-1,-.2,-1.3],[1,-.2,-1.3]].map(([x,y,z],i)=>(<group key={i} ref={wh[i]} position={[x,y,z]}><mesh rotation={[0,0,Math.PI/2]}><cylinderGeometry args={[.3,.3,.24,16]}/><meshStandardMaterial color="#1a1a1a"/></mesh></group>))}
  </group>);
}
...
function Track(){
  ...
  return(<group>
    <mesh rotation={[-Math.PI/2,0,0]} receiveShadow><planeGeometry args={[26,160,32,64]}/><shaderMaterial ref={sm} vertexShader={RVS} fragmentShader={RFS} uniforms={{uT:{value:0}}}/></mesh>
    {Array.from({length:20},(_,i)=>(<React.Fragment key={i}><mesh position={[-14,.5,-75+i*8]}><boxGeometry args={[.5,1,2]}/><meshStandardMaterial color={i%2===0?'#ff3300':'#fff'}/></mesh><mesh position={[14,.5,-75+i*8]}><boxGeometry args={[.5,1,2]}/><meshStandardMaterial color={i%2===0?'#ff3300':'#fff'}/></mesh></React.Fragment>))}
...
function CamRig({pr}){
  ...
  useFrame(()=>{if(!pr.current)return;const p=pr.current.position,r=pr.current.rotation.y;
    camera.position.lerp(new THREE.Vector3(p.x+Math.sin(r)*-9,p.y+5,p.z+Math.cos(r)*-9),.1);
...
function Scene({ctrl,phase,onLap}){
  ...
  useFrame(()=>{if(phase!=='racing'||!pr.current)return;const z=pr.current.position.z;
  ...
  return(<>
    <ambientLight intensity={.4}/><directionalLight position={[10,20,10]} intensity={1.5} castShadow shadow-mapSize={[2048,2048]}/>
    ...
    <Track/>
    <group ref={pr} position={[0,.37,10]}><Car pos={[0,0,0]} color={COLORS[selectedCar]||'#ff3366'} isPlayer ctrl={ctrl} id={0}/></group>
...
export default function Game(){
  ...
  const ctrl=useRef({w:false,a:false,s:false,d:false});const lc=useRef(0);const ts=useRef(0);const[lt,setLt]=useState(0);
  useEffect(()=>{
    const dn=e=>{const k=e.key.toLowerCase();ctrl.current.w=k==='w'||k==='arrowup'?true:ctrl.current.w;ctrl.current.a=k==='a'||k==='arrowleft'?true:ctrl.current.a;ctrl.current.s=k==='s'||k==='arrowdown'?true:ctrl.current.s;ctrl.current.d=k==='d'||k==='arrowright'?true:ctrl.current.d;};
  ...
  const onLap=useCallback(lap=>{lc.current+=1;setBestLap(lap);setLap(lc.current+1);setLt(lap);
    if(lc.current>=totalLaps){const tot=(Date.now()-ts.current)/1000,pos=Math.floor(Math.random()*3)+1,sc=Math.max(0,1000-Math.floor(tot*8)+(4-pos)*180);
      setTotalTime(tot);setPosition(pos);setScore(sc);setRaceResults({lapTime:lap,totalTime:tot,position:pos,score:sc});setPhase('finished');
  ...
  return(<div style={{width:'100%',height:'100%',position:'relative',background:'#0a0a0f'}}>
    <Canvas shadows camera={{position:[0,6,20],fov:65}} style={{position:'absolute',inset:0}}>
      <Suspense fallback={null}><Scene ctrl={ctrl} phase={phase} onLap={onLap}/>
        <EffectComposer><Bloom luminanceThreshold={.6} luminanceSmoothing={.9} intensity={1.2}/><ChromaticAberration offset={[.001,.001]}/><Vignette eskil={false} offset={.3} darkness={.8}/></EffectComposer>
      </Suspense>
    </Canvas>
    {phase==='menu'&&<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.65)',backdropFilter:'blur(6px)'}}>
      <div style={{textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',gap:20}}>
        ...
        <div style={{color:'var(--dim)',fontSize:15}}>WASD or Arrow Keys to drive · 3 Laps</div>
