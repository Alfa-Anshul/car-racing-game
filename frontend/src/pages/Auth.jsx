...
export default function Auth(){
  ...
  const sub=async e=>{e.preventDefault();setErr('');setLoad(true);
    ...
    catch(e){setErr(e.response?.data?.detail||'Failed');}finally{setLoad(false);}}
  const f=k=>e=>setForm(p=>({...p,[k]:e.target.value}));
  return(<div style={{width:'100vw',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--dark)',overflow:'hidden'}}>
    ...
    <motion.div initial={{opacity:0,scale:.9}} animate={{opacity:1,scale:1}} transition={{duration:.4}} style={{width:420,position:'relative',zIndex:10}}>
      <div style={{textAlign:'center',marginBottom:40}}><div className="orb nred" style={{fontSize:34,fontWeight:900}}>TURBO<span className="ncyan">RACE</span></div><div style={{color:'var(--dim)',fontSize:12,marginTop:8,fontFamily:'Orbitron',letterSpacing:'.2em'}}>{mode==='login'?'WELCOME BACK':'JOIN THE GRID'}</div></div>
      <div style={{display:'flex',marginBottom:28,background:'rgba(255,255,255,.03)',border:'1px solid var(--border)'}}>{['login','register'].map(m=>(<button key={m} onClick={()=>setMode(m)} style={{flex:1,padding:'12px 0',fontFamily:'Orbitron',fontSize:11,fontWeight:700,border:'none',cursor:'pointer',background:mode===m?'var(--red)':'transparent',color:mode===m?'#fff':'var(--dim)'}}>{m.toUpperCase()}</button>))}</div>
      <form onSubmit={sub} style={{display:'flex',flexDirection:'column',gap:16}}>
        <div><label style={{fontFamily:'Orbitron',fontSize:10,color:'var(--dim)',letterSpacing:'.15em',display:'block',marginBottom:6}}>USERNAME</label><input className="inp" value={form.username} onChange={f('username')} placeholder="Username" required/></div>
        <AnimatePresence>{mode==='register'&&<motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}}><label style={{fontFamily:'Orbitron',fontSize:10,color:'var(--dim)',letterSpacing:'.15em',display:'block',marginBottom:6}}>EMAIL</label><input className="inp" type="email" value={form.email} onChange={f('email')} placeholder="email@example.com" required/></motion.div>}</AnimatePresence>
        ...
        <motion.button type="submit" className="btn btn-p" disabled={load} whileHover={{scale:1.02}} whileTap={{scale:.98}} style={{marginTop:8,padding:16,fontSize:13,width:'100%'}}>{load?'...':(mode==='login'?'IGNITE ENGINE':'JOIN THE RACE')}</motion.button>
      </form>
      <div style={{textAlign:'center',marginTop:20}}><button onClick={()=>nav('/')} style={{background:'none',border:'none',color:'var(--dim)',cursor:'pointer',fontFamily:'Rajdhani',fontSize:13}}>← Back</button></div>
    </motion.div>
  </div>);
}
